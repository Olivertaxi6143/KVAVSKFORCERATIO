...
def test_debug_asesor_especifico():
    ...
    try:
        ...
        def normalizar_columnas_y_kpis(df):
            """Función local simple para normalizar columnas y KPIs."""
            # Obtener columnas numéricas excluyendo identificadores
            columnas_excluir = ['Strategy_Name', 'Quality_Category', 'Unified_Score', 'Explicación']
            kpis_numericos = [
                col for col in df.columns
                if pd.api.types.is_numeric_dtype(df[col]) and col not in columnas_excluir
            ]
            return df, kpis_numericos
