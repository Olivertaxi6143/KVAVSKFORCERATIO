...
class FlujoCompletoActualizado:
    ...
    def paso3_asesor_financiero(self, estrategias_filtradas: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            estrategias_normalizadas, kpis_disponibles = self._normalizar_columnas_y_kpis(estrategias_filtradas.copy())
    ...
    def _normalizar_columnas_y_kpis(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        """
        Normaliza las columnas de nombres de estrategia y selecciona los KPIs relevantes.
        """
        # Renombrar columnas para que coincidan con el motor de análisis
        new_columns = {}
        for col in df.columns:
            if col.startswith('CAGR_IS'):
                new_columns['CAGR'] = col
            elif col.startswith('CAGR_OOS'):
                new_columns['CAGR_OOS'] = col
            elif col.startswith('Sharpe_Ratio_IS'):
                new_columns['Sharpe_Ratio'] = col
            elif col.startswith('Sharpe_Ratio_OOS'):
                new_columns['Sharpe_Ratio_OOS'] = col
            elif col.startswith('Drawdown_IS'):
                new_columns['Drawdown'] = col
            elif col.startswith('Drawdown_OOS'):
                new_columns['Drawdown_OOS'] = col
            elif col.startswith('Profit_factor_IS'):
                new_columns['Profit_factor'] = col
            elif col.startswith('Profit_factor_OOS'):
                new_columns['Profit_factor_OOS'] = col
            elif col.startswith('Winning_Percent_IS'):
                new_columns['Winning_Percent'] = col
            elif col.startswith('Winning_Percent_OOS'):
                new_columns['Winning_Percent_OOS'] = col
            elif col.startswith('Net_Profit_IS'):
                new_columns['Net_Profit'] = col
            elif col.startswith('Net_Profit_OOS'):
                new_columns['Net_Profit_OOS'] = col
            elif col.startswith('CalmarRatio_IS'):
                new_columns['CalmarRatio'] = col
            elif col.startswith('CalmarRatio_OOS'):
                new_columns['CalmarRatio_OOS'] = col
            elif col.startswith('Max_Consec_Losses_IS'):
                new_columns['Max_Consec_Losses'] = col
            elif col.startswith('Max_Consec_Losses_OOS'):
                new_columns['Max_Consec_Losses_OOS'] = col
            elif col.startswith('Ulcer_Index_IS'):
                new_columns['Ulcer_Index'] = col
            elif col.startswith('Ulcer_Index_OOS'):
                new_columns['Ulcer_Index_OOS'] = col
            elif col.startswith('VaR_95_IS'):
                new_columns['VaR_95'] = col
            elif col.startswith('VaR_95_OOS'):
                new_columns['VaR_95_OOS'] = col
            elif col.startswith('SQN_IS'):
                new_columns['SQN'] = col
            elif col.startswith('SQN_OOS'):
                new_columns['SQN_OOS'] = col
            elif col.startswith('RINAIndex_IS'):
                new_columns['RINAIndex'] = col
            elif col.startswith('RINAIndex_OOS'):
                new_columns['RINAIndex_OOS'] = col
            elif col.startswith('RecoveryFactor_IS'):
                new_columns['RecoveryFactor'] = col
            elif col.startswith('RecoveryFactor_OOS'):
                new_columns['RecoveryFactor_OOS'] = col
            elif col.startswith('Sortino_Ratio_IS'):
                new_columns['Sortino_Ratio'] = col
            elif col.startswith('Sortino_Ratio_OOS'):
                new_columns['Sortino_Ratio_OOS'] = col
            elif col.startswith('CVaR_95_IS'):
                new_columns['CVaR_95'] = col
            elif col.startswith('CVaR_95_OOS'):
                new_columns['CVaR_95_OOS'] = col
            elif col.startswith('Exposure_IS'):
                new_columns['Exposure'] = col
            elif col.startswith('Exposure_OOS'):
                new_columns['Exposure_OOS'] = col
            elif col.startswith('Avg_Bars_in_Trade_IS'):
                new_columns['Avg_Bars_in_Trade'] = col
            elif col.startswith('Avg_Bars_in_Trade_OOS'):
                new_columns['Avg_Bars_in_Trade_OOS'] = col
            elif col.startswith('Max_Drawdown_Duration_IS'):
                new_columns['Max_Drawdown_Duration'] = col
            elif col.startswith('Max_Drawdown_Duration_OOS'):
                new_columns['Max_Drawdown_Duration_OOS'] = col
            elif col.startswith('Payout_Ratio_IS'):
                new_columns['Payout_Ratio'] = col
            elif col.startswith('Payout_Ratio_OOS'):
                new_columns['Payout_Ratio_OOS'] = col
            else:
                new_columns[col] = col # Mantener nombres originales si no se mapea
        
        # Seleccionar solo las columnas que existen en el DataFrame original
        # y que han sido renombradas o son nombres de estrategia
        selected_cols = [new_columns.get(col, col) for col in df.columns]
        
        # Crear un nuevo DataFrame con las columnas renombradas y seleccionadas
        df_normalized = df[selected_cols]
        
        # Identificar las columnas que son KPIs numéricos
        kpis_disponibles = [col for col in df_normalized.columns if pd.api.types.is_numeric_dtype(df_normalized[col])]
        
        return df_normalized, kpis_disponibles
