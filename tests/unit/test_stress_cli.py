...
try:
    print('[INFO] Cargando archivo KPI con delimitador ";" y quotechar " ...')
    df_kpi = pd.read_csv(kpi_path, sep=';', quotechar='"', engine='python')
    print(f"[INFO] Columnas leídas: {list(df_kpi.columns)}")
    ...
    if faltantes:
        print(f"[ERROR] Faltan columnas requeridas en el archivo KPI: {faltantes}")
        sys.exit(1)
    print(f"✅ Archivo KPI cargado correctamente: {df_kpi.shape}")
    ...
    print("[INFO] Ejecutando análisis robusto y científico...")
    ...
    print(summary)
    print(f"[INFO] Estrategias analizadas: {len(results)}")
    ...
    print(f"[INFO] Estrategias seleccionadas para exportar: {len(seleccionadas)}")
    ...
    for nombre in nombres:
        nombre_str = str(nombre)  # Asegura que siempre sea string
        posibles = [f for f in os.listdir(sqx_folder) if nombre_str in f and f.endswith('.sqx')]
        if posibles:
            print(f"[INFO] Copiando archivos .sqx para estrategia: {nombre_str} -> {posibles}")
        for sqx in posibles:
            shutil.copy2(os.path.join(sqx_folder, sqx), os.path.join(dest_folder, sqx))
            copiados += 1
    print(f"✅ Archivos .sqx copiados a {dest_folder}: {copiados}")
    print("[INFO] Flujo de análisis y exportación completado con éxito.")
    ...
    # --- Corrección profesional: nunca usar sys.exit en tests, usar assert False ---
    assert False, f"Error crítico en test_stress_cli.py: {e}" 