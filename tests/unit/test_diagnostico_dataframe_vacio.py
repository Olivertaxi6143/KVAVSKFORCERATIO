...
def test_diagnostico_completo():
    ...
    try:
        ...
        # PASO 3: Test DataManager (reemplaza DataLoaderEnhanced)
        print("\n🔄 PASO 3: Test DataManager")
        print("-" * 50)
        
        data_manager = DataManager()
        success = data_manager.load_kpis_data(kpi_file)
        
        if success and data_manager.kpis_data is not None:
            df_loader = data_manager.kpis_data
            print(f"✅ DataManager: {len(df_loader)} filas, {len(df_loader.columns)} columnas")
            print(f"📋 Columnas DataManager: {list(df_loader.columns)[:10]}...")
        else:
            print("❌ ERROR: DataManager no pudo cargar los datos")
            return False
        ...
        results, summary = run_complete_analysis_with_gui_integration(
            pd.read_csv(kpi_file, sep=';', decimal=','),
            config=config
        )
        ...
        print(f"✅ DataManager: {len(df_loader)} filas")
