...
from src.core.utils.data_utils import (
    safe_sum, safe_values, safe_float, improve_missing_data_handling,
...
from src.core.utils.type_converters import (
    convert_types, safe_int, safe_str, safe_bool, convert_series_types,
    convert_dataframe_types, validate_types, infer_numeric_type,
    normalize_numeric_series, ensure_numeric_columns, convert_to_datetime,
...
def test_all_utils_integration():
    ...
    try:
        ...
        # 4. Normalizar serie
        normalized = normalize_series(df_cleaned['B'].astype(float))
        assert normalized.min() == 0
        assert normalized.max() == 1
